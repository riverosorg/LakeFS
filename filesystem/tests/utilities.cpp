...
TEST_CASE("Query Extraction", "[utilities]")
{
    ...
    {
        auto res = extract_query("/test/wow/l/(tag1 | tag2)");

        CHECK(res == "(tag1 | tag2)");

        res = extract_query("/test/wow/l/(tag1 | tag2)/file");
    ...
    SECTION("Subquerying")
    {
        auto res = extract_query("/test/wow/l/(tag1 & (!place))");

        CHECK(res == "(tag1 & (!place))");

        res = extract_query("/test/wow/l/(tag1 & (!place))/file");
    ...
    {
        auto res = extract_query("/test/wow/l/(tag1 & ((!place) | (tag2 & tag3)))");

        CHECK(res == "(tag1 & ((!place) | (tag2 & tag3)))");

        res = extract_query("/test/wow/l/(tag1 & ((!place) | (tag2 & tag3)))/file");
