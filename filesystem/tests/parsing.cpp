...
TEST_CASE("Parsing text into a AST query", "[parsing]")
{
    ...
    SECTION("Single Letter Tags")
    {
        ast = parse("a&b").value();
        expected_ast =
            std::make_shared<Intersection>(std::make_shared<Tag>("a"), std::make_shared<Tag>("b"));
        REQUIRE(ast->match(expected_ast));

        ast = parse("a&(b|c)").value();
        expected_ast = std::make_shared<Intersection>(
            std::make_shared<Tag>("a"),
            std::make_shared<Union>(std::make_shared<Tag>("b"), std::make_shared<Tag>("c")));
        REQUIRE(ast->match(expected_ast));
    ...
    SECTION("Word Tags")
    {
        ...
        expected_ast = std::make_shared<Intersection>(std::make_shared<Tag>("tag1"),
        ...
        expected_ast = std::make_shared<Intersection>(
            ...
            std::make_shared<Union>(std::make_shared<Tag>("tag2"), std::make_shared<Tag>("tag3")));
        REQUIRE(ast->match(expected_ast));
    }
    ...
    SECTION("Complex Queries")
    {
        ...
        expected_ast = std::make_shared<Intersection>(
            std::make_shared<Union>(std::make_shared<Tag>("picture"),
                                    std::make_shared<Tag>("video")),
            std::make_shared<Intersection>(
                std::make_shared<Tag>("year_2014"),
                std::make_shared<Negation>(std::make_shared<Tag>("digital"))));
