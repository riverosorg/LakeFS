...
TEST_CASE("Basic Tag Retrieval", "[parsing][query]")
{
    ...
    SECTION("Single Tag Retrieval")
    {
        ...
        expected_query = "SELECT path FROM data WHERE id IN "
    ...
    SECTION("Negation")
    {
        ast = parse("!tag").value();
        query = db_create_query(ast);
        expected_query = "SELECT path FROM data WHERE id NOT IN "
                         "(SELECT data_id FROM tags WHERE tag_value = 'tag');";
    ...
    SECTION("Union Retrieval")
    {
        ...
        query = db_create_query(ast);
        expected_query = "SELECT path FROM data WHERE id IN "
                         "(SELECT data_id FROM tags WHERE tag_value = 'tag1' UNION SELECT "
                         "data_id FROM tags WHERE tag_value = 'tag2');";
