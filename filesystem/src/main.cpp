...
auto main(int argc, char** argv) -> int
{
    ...
    program.add_argument("mount_point").required().help("The directory to mount the filesystem at");

    program.add_argument("default_query")
        .default_value("default")
        .help("Specify the initial default query of the mount");

    program.add_argument("--tempdb").flag().help("Use an in-memory database instead of the file");

    program.add_argument("--config", "-c")
        .default_value("/etc/lakefs.conf")
        .help("Manually set a config location rather than using "
              "/etc/lakefs.conf");

    program.add_argument("-f").flag().help("Run program in foreground rather than as a daemon");

    program.add_argument("-d").flag().help("Output complete debug information while running");
    ...
    const auto config_path = std::filesystem::absolute(program.get<std::string>("--config"));
    ...
    // Extract mount point
    const auto mount_point = std::filesystem::absolute(program.get<std::string>("mount_point"));
    ...
    else
    {
        ...
        {
            // Create backup timer
            if (create_backup_timer(interval.value(), std::stoi(config["max_backups"]),
                                    config["dir"]))
            {
            ...
            {
                spdlog::warn("Could not start backup system. Continuing "
                             "without backups");
            }
        }
    }
