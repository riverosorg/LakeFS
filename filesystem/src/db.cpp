...
int db_tmp_init()
{
    ...
    rc = sqlite3_exec(db, "CREATE TABLE tags (data_id INTEGER, tag_value TEXT);", nullptr, nullptr,
                      nullptr);

    rc = sqlite3_exec(db, "CREATE TABLE data (id INTEGER PRIMARY KEY, path TEXT);", nullptr,
                      nullptr, nullptr);
...
int db_create_backup(const std::string backup_path)
{
    ...
    while (backup == NULL)
    {
        ...
        // spdlog::error("Could not initialize backup: {0}",
...
{
    int rc = sqlite3_exec(db, ("INSERT INTO data (path) VALUES ('" + path + "');").c_str(), nullptr,
                          nullptr, nullptr);
...
int db_tag_file(const std::string path, const std::string tag)
{
    ...
    int rc = sqlite3_exec(db,
                          ("INSERT INTO tags (data_id, tag_value) VALUES "
                           "((SELECT id FROM data WHERE path = '" +
                           path + "'), '" + tag + "');")
                              .c_str(),
...
int db_remove_file(const std::string path)
{
    int rc = sqlite3_exec(
        db,
        ("DELETE FROM tags WHERE data_id = (SELECT id FROM data WHERE path = '" + path + "');")
            .c_str(),
        nullptr, nullptr, nullptr);
    rc = sqlite3_exec(db, ("DELETE FROM data WHERE path = '" + path + "';").c_str(), nullptr,
                      nullptr, nullptr);
...
{
    int rc = sqlite3_exec(db,
                          ("DELETE FROM tags WHERE data_id = (SELECT id FROM "
                           "data WHERE path = '" +
                           path + "') AND tag_value = '" + tag + "';")
                              .c_str(),
                          nullptr, nullptr, nullptr);
...
{
    int rc = sqlite3_exec(
        db, ("UPDATE data SET path = '" + new_path + "' WHERE data.path = '" + path + "';").c_str(),
        nullptr, nullptr, nullptr);

    return rc;
}

// Recursively generate a SQL query string for WHERE clause of the standard
// query
...
void db_set_default_query(const std::string query) { default_query = query; }
